...
layui.use(['layer', 'form', 'jquery'], function () {
    ...
    function insert_section(uid) {
        ...
        var from_id = getPar("kefu");
        get_message(from_id,uid);
    }
    ...
    function update_online_status() {
        ...
        if(my_csr_online > num){
            num = online_num;
        }
        $(".friend-head-right").html( my_csr_online + ' / ' + num + ' 人' );
    }
    ...
    function get_users() {
        uuid = getPar("kefu");
        var req_url = '/users?uuid='+ uuid;
        $.get(req_url,function (data) {
            if(data.code == 200){
                $('.chat-user').html('');
                var data = data.data
                for (i=0;i<data.length;i++){
                    insert_user_html(data[i],"客户" + '#'+ (uuids.length + 1));
                    //创建聊天section
                    insert_section(data[i]);
                    uuids.push(data[i]);
                    my_csr_online += 1;
                }
    ...
    //获取最新的五条数据
    function get_message(from_id,to_id) {
        $.get('/message?from_id='+from_id+'&to_id='+from_id,function (data) {
            if(data.code == 200){
                data.data.reverse().forEach(function (msg) {
                    if(msg.from_uid == to_id){
